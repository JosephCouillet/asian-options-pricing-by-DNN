...
from ml_model import MLP  # your model definition
import joblib 
...
# Load model weights
state = torch.load('models/mlp_weights.pt')
model = MLP()
...
with torch.no_grad():
    y_pred_scaled = model(torch.from_numpy(X_scaled).float()).numpy()
...
H, xedges, yedges = np.histogram2d(
    df['K'], df['T'], bins=[kbins, tbins],
    weights=errors, density=False
)
